...
#TODO: make static push/pull work
#TODO: implement chapter 8 effects
...
class CodeWriter():
	...
	def writeMemoryAccess(self, command, segment, variable):
		...
		segmentTranslate = {"static": "static", "pointer": "pointer", "temp": "temp", "constant": "constant", "local": "LCL", "argument": "ARG", "this": "THIS", "that": "THAT"} 
		...
		elif((segment == "static") and (command == "push")):
			address = getAddress(segment, variable)
			asmCode = "\n//this is a push static translation\n@"+ variable +"\nD=M\n@SP\nA=M\nM=D\n@SP\nM=M+1\n"
			self.asmFile.write(asmCode)
		elif((segment == "static") and (command == "pop")):
			asmCode = "@SP\nM=M-1\n@" + variable + "\nD=A\n@3\nD=A+D\n@temp\nM=D\n@SP\nA=M\nD=M\n@temp\nA=M\nM=D\n"
			self.asmFile.write(asmCode)
		
		#TODO: eliminate redundancies. Note pointer and temp have same assembly code pretty much
		elif((segment == "pointer") and (command == "push")):
			asmCode = "@"+variable+"\nD=A\n@3\nD=A+D\n@temp\nM=D\n@temp\nA=M\nD=M\n@SP\nA=M\nM=D\n@SP\nM=M+1\n"
			self.asmFile.write(asmCode)
		elif((segment == "pointer") and (command == "pop")):
			asmCode = "@SP\nM=M-1\n@" + variable + "\nD=A\n@3\nD=A+D\n@temp\nM=D\n@SP\nA=M\nD=M\n@temp\nA=M\nM=D\n"
			self.asmFile.write(asmCode)
	...
	def getAddress(segment, variable):
		...
		if(segment == "temp"):
			...
			address = (str)(sys.argv[1][:-3]) + "." + (str)(self.staticCount)
			self.staticCount =+1	
		else:
